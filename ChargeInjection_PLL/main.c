...
interrupt void ADC_interrupt1(void)
{
	...
	//theta_rad = (theta + delta)*2*3.14159265359;
   if( ((theta + delta) > 0.249) && ((theta + delta) < 0.251) ) //if theta = pi/2, get maximum
	...
	else if (control_mode == controlmode_dynamic)
	{
		...
		Vc2_ref_V = 1.4*Vc3_rip_V*0.5; // Scaled
	...
	//test = theta*2048;
	//test = (notch_out_pll)*Vc1_adc_fullvolt_to_count_ratio;
	test = (Vc3_rip_V)*Vc1_adc_fullvolt_to_count_ratio;
