...
interrupt void ADC_interrupt1(void)
{
	...
	/*if (Vc3_V*Vc3_prev_V < 0) //&& (Vc3_max_count > Vc3_max_count*0.1)) // zero-crossing
	{
	...
	/*if (control_mode == controlmode_fixed)
	{
		...
		M_control = Vc3_ref_V*Vc2_ref_init_V_div; 	// Desired conversion ratio of converter, Vout/Vin = Vc3/Vc2 (fixed Vc2_ref = 20V)
...
//	DacaRegs.DACVALS.bit.DACVALS = (int16) Vbus_count+2048;
	DacaRegs.DACVALS.bit.DACVALS = (int16) (Vc1_count)*gain_dac +2048;
